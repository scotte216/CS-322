...
class IR1Gen {
    ...
    static List<IR1.Inst> gen(Ast1.If n) throws Exception {
        ...
        //Optimization: If condition is a boolean, we can evalute immediately
    ...
    static List<IR1.Inst> gen(Ast1.While n) throws Exception {
        ...
        // Optimization: If condition is BoolLit false, ignore the body of the while loop (stmt)
        if (cond.src instanceof IR1.BoolLit && !((IR1.BoolLit)cond.src).b)
        {
            return code;
        }
    ...
    static CodePack genAddr(Ast1.ArrayElm n) throws Exception {
        ...
        int offset = ((IR1.IntLit)e2.src).i * 4;
        
        code.add(new IR1.Binop(IR1.AOP.ADD,t1, e1.src, new IR1.IntLit(offset))); 
    ...
    static CodePack gen(Ast1.ArrayElm n) throws Exception {
        ...
        //Optimization: evalute index * for immediately for intlits
        if (e2.src instanceof IR1.IntLit)
        {
            int offset = ((IR1.IntLit)e2.src).i * 4;
            code.add(new IR1.Binop(IR1.AOP.ADD, t1, e1.src,new IR1.IntLit(offset)));
            code.add(new IR1.Load(t2, new IR1.Addr(t1)));
            return new CodePack(t2,code);
        }
        //Evaluate index by normal means.
        else
        {
    ...
    static CodePack gen(Ast1.Binop n) throws Exception {
        ...
        // Evaluate binop for special case: And / Or
        if (bop == IR1.AOP.OR || bop == IR1.AOP.AND) {
            return genAndOrBinOp(n, code, bop);
        } else {
            CodePack e1 = gen(n.e1);
            CodePack e2 = gen(n.e2);

            //Optimization: if e1.src and e2.src are integers, just add them and 
            //return it.
            if (e1.src instanceof IR1.IntLit && e2.src instanceof IR1.IntLit) {
                return doIntLitOptimization(bop, e1, e2);
            }
            //Optimization: If e1.src and e2.src are booleans, just return the 
            //result of the binop.
            else if (e1.src instanceof IR1.BoolLit && e2.src instanceof IR1.BoolLit) {
                return doBoolLitOptimization(bop, e1, e2);
            }
            //else do regular binop code execution
            else {
    ...
    // Optimization: routine for if both expressions in a binop are boolean literals.
    // Evaluate immediate result.
    private static CodePack doBoolLitOptimization(IR1.BOP bop, CodePack e1, 
                    CodePack e2) throws Exception {
        boolean a = ((IR1.BoolLit) e1.src).b;
        boolean b = ((IR1.BoolLit) e2.src).b;
        Ast1.BoolLit result = null;

        if (bop == IR1.ROP.EQ) result = new Ast1.BoolLit(a == b);
        if (bop == IR1.ROP.NE) result = new Ast1.BoolLit(a != b);

        return gen(result);
    }

    // Optimization: routine for if both expressions in a binop are integer literals.
    // Evalute immediate result.
    private static CodePack doIntLitOptimization(IR1.BOP bop, CodePack e1, 
            CodePack e2) throws Exception {
        if (bop instanceof IR1.AOP)
            return gen(helper((IR1.AOP) bop, (IR1.IntLit) e1.src, 
                              (IR1.IntLit) e2.src));
        else
            return gen(helper((IR1.ROP) bop, (IR1.IntLit) e1.src, 
                              (IR1.IntLit) e2.src));
    }

    //Generate codepack for the And and Or binop
    private static CodePack genAndOrBinOp(Ast1.Binop n, List<IR1.Inst> code, 
                    IR1.BOP bop) throws Exception {
        IR1.Temp temp;
        ...
        // Optimization: Relational operator for boolean literals
        if (e1.src instanceof IR1.BoolLit && e2.src instanceof IR1.BoolLit) {
            return doAndOrOptimizationTwoBoolLit(operator, e1, e2);
        }

        // Optimization: first term is boolean literal
        if (e1.src instanceof IR1.BoolLit)
        {
            return doAndOrOptimizationOneBoolLit(operator, e1, e2);
        }

        if (e2.src instanceof IR1.BoolLit)
        {
            return doAndOrOptimizationOneBoolLit(operator, e2, e1);
        }
        //Otherwise do regular code evaluation shortcuts for the And and Or binop 
        //expressions
    ...
    private static CodePack doAndOrOptimizationOneBoolLit(IR1.BoolLit operator, CodePack e1, CodePack e2) throws Exception {
        boolean value = ((IR1.BoolLit)e1.src).b;

        // This logic could be simplified, but for ease of readability and
        // self documentation, I'm keeping it explicitly spelled out.
        if (operator.b) // operator true means OR
        {
            // First term is true, return true;
            if (value)
                return gen(new Ast1.BoolLit(true));
        ...
        {
            //And the first term is true, return second expression
            if (value)
            {
                return e2;
            }
            //else first term is false, return false
            else
            {
                return gen(new Ast1.BoolLit(false));
            }
        }
    }

    // Optimization: If both expressions in an And/Or binop are boolean literals, 
    // generate immediate result.
    private static CodePack doAndOrOptimizationTwoBoolLit(IR1.BoolLit operator, CodePack e1,
                                                          CodePack e2) throws Exception {
        boolean a = ((IR1.BoolLit) e1.src).b;
        boolean b = ((IR1.BoolLit) e2.src).b;
        Ast1.BoolLit result;
    ...
    static CodePack gen(Ast1.Unop n) throws Exception {
        ...
        // Optimization: If the expression is a boolean literal, simply generate 
        // the opposite value
        ...
        // Optimization:
        // If the expression is an integer literal, simply generate the opposite
        // value
        // ERROR -- Interpreter can't handle negative integer values as array
        // assignment.
        if (e.src instanceof IR1.IntLit && uop == "-")
        {
            int value = -((IR1.IntLit)e.src).i;
            return gen(new Ast1.IntLit(value));
        }
